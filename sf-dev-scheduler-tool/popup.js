...
async function loadOrgOptions() {
  ...
  state.selectedOrg = state.availableOrgs.find(org => org.isActive)
...
function updateStats(derived) {
  ...
  setText("runsToday", String(new Set([
    ...derived.asyncToday.map(job => job.id),
    ...derived.ranTodaySchedules.map(job => job.id)
  ]).size));
...
function getViewConfig(derived) {
  const runRows = applyRowFilters(derived.asyncToday.map(job => ({
    ...
    note: job.numberOfErrors ? `${job.numberOfErrors} errors` : "Completed"
  })));
...
function resolveAsyncSchedulerName(asyncJob) {
  if (asyncJob.schedulerName && asyncJob.schedulerName !== asyncJob.apexClassName) {
    return asyncJob.schedulerName;
  }

  const directMatch = findScheduleByTimestamp(asyncJob.createdDate);
  if (directMatch?.schedulerName) {
    return directMatch.schedulerName;
  }

  return "Scheduler unavailable";
}

function findScheduleByTimestamp(timestamp) {
...
function getRelatedRuns(job) {
  return state.asyncJobs
    .filter(run => resolveAsyncSchedulerName(run) === job.schedulerName)
    .sort((left, right) => new Date(right.createdDate || 0) - new Date(left.createdDate || 0));
}

function getRelatedApexClass(job) {
  return getRelatedRuns(job).find(run => run.apexClassName && run.apexClassName !== "-")?.apexClassName || "-";
}
...
function updateMeta(force) {
  const parts = [];
  if (state.lastSuccessfulFetchAt) {
    parts.push(`Last updated: ${DATE_TIME_FORMATTER.format(new Date(state.lastSuccessfulFetchAt))}`);
  }
  parts.push(`Schedules: ${state.scheduledApexJobs.filter(job => String(job.jobType || "").toLowerCase() === "scheduled apex").length}`);
  parts.push(`Today runs: ${state.asyncJobs.filter(job => isToday(job.createdDate, getDisplayTimeZone())).length}`);
  parts.push(`Due today: ${state.scheduledApexJobs.filter(job => isRunnableToday(job, getDisplayTimeZone(), new Date())).length}`);
