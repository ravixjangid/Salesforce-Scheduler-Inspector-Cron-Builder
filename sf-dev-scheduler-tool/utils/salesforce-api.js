...
async getActiveSalesforceTab() {
  ...
  if (!this.isSalesforceHost(url.hostname)) {
    throw new Error("Open Salesforce in the active Chrome tab, then refresh.");
  }
...
async getSalesforceTabs() {
  ...
  return tabs
    ...
    .filter(Boolean)
    .filter(entry => this.isSalesforceHost(entry.url.hostname))
...
async runQueryWithMeta(query, context = {}) {
  ...
  for (const origin of origins) {
    try {
      ...
      const payload = await this.fetchJson(origin, query, sidCookie?.value || null);
      lastPayload = payload;
      lastOrigin = origin;
...
async fetchJson(origin, query, sid) {

  const url = `${origin}/services/data/${this.API_VERSION}/query?q=${encodeURIComponent(query)}`;
  const headers = {
