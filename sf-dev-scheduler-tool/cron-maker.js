...
function initCronMaker() {
  ...
  document.getElementById("runCount").addEventListener("change", handleRunCountChange);
...
function handleRunCountChange() {
  setActivePreset("");
  applyDistinctDefaultsForFrequency(document.getElementById("frequency").value);
  updateCronState();
}
...
function resetBuilderInputs(frequency = "hourly") {
  ...
  applyDistinctDefaultsForFrequency(frequency);
}
...
function applyDistinctDefaultsForFrequency(frequency) {
  const runCount = Number(document.getElementById("runCount")?.value || 1);

  if (frequency === "daily") {
    applyDistinctDefaults(["hour", "hour2", "hour3"], runCount);
    return;
  }

  if (frequency === "weekly") {
    applyDistinctDefaults(["weekday", "weekday2", "weekday3"], runCount);
    return;
  }

  if (frequency === "monthly") {
    applyDistinctDefaults(["dayOfMonth", "dayOfMonth2", "dayOfMonth3"], runCount);
    return;
  }

  if (frequency === "yearly") {
    applyDistinctDefaults(["month", "month2", "month3"], runCount);
  }
}

function applyDistinctDefaults(ids, count) {
  ...
  ids.slice(0, count).forEach(id => {
    ...
    const fallbackOption = Array.from(select.options).find(option => !usedValues.has(option.value));

    if (fallbackOption) {
      select.value = fallbackOption.value;
      usedValues.add(fallbackOption.value);
      return;
    }
...
function updateCronState() {
  ...
  if (scopeSection) {
    scopeSection.style.display = showMonthlyRuns || showYearlyRuns ? "block" : "none";
  }
