importScripts("utils/salesforce-host-support.js", "utils/salesforce-api.js");
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message?.type !== "CHECK_SALESFORCE_SESSION") {
    return undefined;
  }

  checkSalesforceSession(message.tabUrl || sender?.tab?.url || "")
...
async function checkSalesforceSession(tabUrl) {
  if (!tabUrl) {
    return false;
  }
  ...
  try {
    ...
    origins = globalThis.SalesforceAPI?.getCandidateApiOrigins
      ? SalesforceAPI.getCandidateApiOrigins(tabUrl)
      : [url.origin];
  }
  catch (error) {
    return false;
  ...
  for (const origin of origins) {
    const cookie = await getCookie(origin, "sid");
    if (cookie?.value) {
      return true;
    }
  }
  ...
  return cookies.some(cookie => isMatchingSessionCookie(cookie, host));
}
